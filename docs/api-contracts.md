...
| `POST /api/streams/{id}/extend/` | 延長期限を更新し、同じ期限の新 publish JWT を発行 | 本人（所有者） | `ExtendStreamResponse.whipUrl` は開始時と同じ publish / resource 操作先。`STREAM_EXTENSION_ENABLED=true` の時だけ `ExtendStreamResponse` を返す。`false`（既定）では期限・JWTを更新せず409 `STREAM_EXTENSION_DISABLED`。終了済みは409 `STREAM_ENDED` |
