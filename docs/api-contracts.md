...
| `POST /api/streams/` | 新しい 12 文字 path ID と publish JWT を発行、または終了済み ID を再利用 | 本人 | `CreateStreamRequest` の `id` は任意。指定時は所有する終了済み ID だけを同じ作成間隔・上限・開始 token 制約で再開し、不可は 409 `STREAM_ID_NOT_REUSABLE`。未指定は旧クライアント互換。本人の同時配信は 409 `STREAM_ALREADY_LIVE`、取り消し済み token は 409 `STREAM_START_CANCELLED`、全体 20 本到達は 429 `STREAM_CAPACITY_REACHED`、作成間隔内は 429 `STREAM_CREATE_RATE_LIMITED` |
