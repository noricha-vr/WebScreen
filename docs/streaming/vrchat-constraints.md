...
コーデックは本番の **H.264 + AAC / MP4 が最大公約数**。RTSPTのOpusとG.711はAVProが映像だけを選択した。MP3候補は実Mac動的H.264でH.264 + MPEG-1/2 Audioの2トラックを選択し、PC映像と低遅延を確認した。30分連続配信と実Mac 24/30 fps比較は合格したが、PC/Quest別の統制済み実聴とQuestのMP3音声付1秒未満は未確認のためAACから昇格しない（2026-09-02実測）。
...
| YamaStream（RTSPT H.264/MP3候補） | **16標本すべて1秒未満、中央値0.151秒**。映像と2トラック選択は確認、PC/Quest別の統制済み実聴とQuestの1秒未満は未確認 |
