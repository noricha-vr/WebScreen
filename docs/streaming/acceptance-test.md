...
| 論点 | 補足 |
|---|---|
| PC の遅延をどう訴求するか | `Use Low Latency` はワールド作者が焼き込む値で**こちらから制御できない**。「対応ワールドなら 0.1 秒級」としか言えず、対応ワールドの案内が要る |
| Quest を出すか | PC と同じ `rtspt://` で映像・AAC 音声を確認済み。URL は共通にし、「PC は超低遅延、Quest は数秒遅れ」と明示する |
