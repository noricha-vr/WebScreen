...
| **A3** | `Use Low Latency` **ON** のワールドで遅延を実測 | **1 秒未満**（既存実測は 0.08 秒） | 現在のワールドの設定値も記録し、満たさなければ訴求文言を変える |
...
| 論点 | 補足 |
|---|---|
| PC の遅延をどう訴求するか | `Use Low Latency` はワールド作者が焼き込む値で**こちらから制御できない**。ON かつ 1 秒未満を確認したワールドだけ「1 秒未満（既存実測は 0.1 秒級）」と案内する |
