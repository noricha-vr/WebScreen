...
| | B1.5 | 29.9 | 1.52 | 35.0 | 26.0 | 0 | 1280x624（末尾 12 秒は取得元が 1042x720 に変わり出口 782x540 = 0.75 倍。`qualityLimitationReason` は none） | — |
...
| S1 heavy（色ノイズのパン。帯域飢餓の最悪ケース） | A | 1.3 | 1.20 | 24.4 | 1.0 | 121 / 160 | 1280x624 | 2.79 s |
| | B1.5 | 2.0 | 1.46 | 22.1 | 1.6 | 107 / 180 | 910x444（84% の時間） | 1.29 s |
...
1. **実ページのスクロール（S2）では A と B の fps・freeze は同じ。** 1.2 Mbps でも A は 29.7 fps を保ち、Issue の前提「帯域不足時に `maintain-resolution` が fps を落とす」は起きなかった。エンコーダは fps ではなく QP（36）で吸収した。B1.5 は +0.3 Mbps で QP を 1 下げただけ（末尾の 782x540 は取得元の寸法変化に伴う 0.75 倍で、帯域起因ではない）
...
3. **B の弱点は解像度が落ちて戻らないこと。** 根拠は 10 分 run の stab-yt-B1.5（帯域低下 56 秒で 910x444 へ落ち、その後 5 分以上戻らず）と S1。一度落ちると静止画では戻す契機がなく、講座用途の文字可読性を保証できない
...
- リアルタイム優先を **現段階では採用しない**。同一素材で A と差が出ず、解像度が落ちて戻らないリスクだけが増える。`selectMode` は UI から削除済み（#208）。検証用 query gate だけを残す。
...
- **Cherry に `source` 方式を反映して再計測（run C'、2026-09-05 00:08 JST、8 分）**: Indigo origin → Cherry replica（`source: rtsp://stream.web-screen.net:554/live/$G1`）→ Mac の出口は **中央値 0.944〜0.964 秒** / p95 0.96〜1.24 秒（生値 `docs/tmp/latency/2026-09-04T15-08-19-045Z`）。Cherry の egress（server-snap）は 0.848〜0.853 秒で、Indigo egress（A の 0.79〜0.82 秒）との差は日本 → Chicago の片道ぶんだけ。**replica 経由の出口が Cherry origin 直（B の 0.951 秒）と同じ**になり、I26 で +0.31 秒（推定 1.18 秒）だった pull ホップのコストは計測誤差の範囲で消えた。run D（同一 DC 内 replica、origin egress を 5554 に退避する構成）はローカル実測 0.3 ms と run C' で十分なため再実施していない。配信元は改修後の計測ページ（秒識別ビープに傾斜、tones=1 は 220/330 Hz）で、`outlet-audio.csv` 478 標本と A/V 差 365 ms が取れておりビープ検出は変わらず動く
- **run A'（Indigo 直、改修後ページ、`--player win2022`、2026-09-05 00:58 JST、8 分）**: Mac からの出口は中央値 **0.778〜0.781 秒**（I26 A の 0.776 秒と一致。生値 `docs/tmp/latency/2026-09-04T15-58-50-423Z`）。server-snap は ingress 0.54 秒 / egress 0.78〜1.04 秒。VRChat プレイヤー側の Windows 録画は開始 16 秒で ffmpeg がコンソール閉鎖の SIGTERM を受けて止まり標本 0（URL をプレイヤーに貼った直後。原因の切り分けは [#250](https://github.com/noricha-vr/WebScreen/issues/250)）。なお計測ページは秒針を描くようになったが `draw()` は 250 ms 間隔のままなので送出は引き続き 4 fps で、relay コスト約 0.25 秒（ingress → egress）も I26 と同じ
- **run A''（Indigo 直、`--stream-id` で URL 固定、`--player win2022`、2026-09-05 13:08 JST、10 分）: VRChat プレイヤーの標本が初めて全区間で取れた**（1800 標本/分 × 10 分。生値 `docs/tmp/latency/2026-09-05T04-08-22-041Z`）。出口は中央値 0.775〜0.777 秒で A' と同じ。**プレイヤーは貼付後 15 秒が約 0.35 秒、約 50 秒で 1.12 秒へ 1 段上がりそのまま定常（中央値 1.12〜1.15 秒、p95 1.25〜1.27 秒）**。出口との差 +0.37 秒は AVPro 側のバッファ拡張と見られ、I24 追記の「0〜25 秒で 224〜291 ms」は立ち上がり区間だけを見ていた値。A' の標本 0 は「貼付が原因」ではなかった（同日 11:26 JST の再現では win2022 のコミット枯渇（Docker Desktop の backend が 99 GB 確保 → dwm.exe クラッシュ）で gdigrab と VRChat が落ちたが、A' の signal 15 の時刻帯にはその痕跡がなく原因は未解明）。Docker 停止・物理モニター点灯・ワールド入り直し後の貼付・正面固定、の 4 条件で完走した（[#250](https://github.com/noricha-vr/WebScreen/issues/250)）
