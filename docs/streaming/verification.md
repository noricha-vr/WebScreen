...
`web/scripts/latency-probe.ts`（[latency-harness.md](latency-harness.md)）で、ms25 の実 Chrome から計測ページ（ms 時刻のブロックコード + 毎秒ビープ）を製品経路で本番配信し、出口を測った。

| 測定点 | 方法 | 結果 |
|---|---|---|
| Mac → egress 出口 | Mac から単発取得（約 4 秒間隔、2 分） | 上限値 中央値 **0.78 秒**（p95 0.79 秒）。開始 2 秒後から一定 |
| 配信元 → ingress（サーバー内） | サーバー内で単発取得 | 約 **0.55 秒** |
| relay（ingress → egress、サーバー内） | 同時刻の差 | 約 **+0.25 秒**（単発取得の時間差込み。合成配信の relay 単体は 40〜92 ms） |
| 音声 | 毎秒ビープの到着位相 | 1 秒剰余で約 0.26〜0.47 秒（絶対値は映像との近接標本から 1 秒未満と判断） |

連続 ffmpeg（`-vf fps=5` / `-use_wallclock_as_timestamps 1`）で測ると 2〜6 秒の一定遅延に見えたが、サーバー内の単発取得と食い違い、起動時の PTS ギャップと内部バッファによる測定アーティファクトだった。**出口には「開始直後 3 秒 → 1 秒未満」の収束は無く**、体感で報告されたその挙動は VRChat（AVPro）側に残る。プレイヤー側の時系列（Windows 録画 + ブロックコード復号）はハーネスに実装済みで、VRChat に URL を貼った状態での採取が次の手順。
