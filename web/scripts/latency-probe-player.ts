...
export async function recordWindowsPlayer(outDir: string, until: number): Promise<PlayerResult> {
  ...
  const command = `$ErrorActionPreference='Stop'; $ff='${ffmpeg}'; $out='${remote}'; $log='${log}'; $startedPath='${started}'; $donePath='${started}.done'; $task='WebScreenLatencyHarness'; Remove-Item $startedPath,$donePath -Force -ErrorAction SilentlyContinue; $taskScript="\`$recorded=[DateTime]::UtcNow; Set-Content -Path '$startedPath' -Value \`$recorded.ToString('o') -NoNewline; & '$ff' -y -f gdigrab -framerate 30 -i desktop -t ${seconds} '$out' 2>&1 | Set-Content -Path '$log'; Set-Content -Path '$donePath' -Value \`$LASTEXITCODE -NoNewline; exit \`$LASTEXITCODE"; $encoded=[Convert]::ToBase64String([Text.Encoding]::Unicode.GetBytes($taskScript)); $action=New-ScheduledTaskAction -Execute 'powershell.exe' -Argument "-NoProfile -EncodedCommand $encoded"; $principal=New-ScheduledTaskPrincipal -UserId $env:USERNAME -LogonType Interactive -RunLevel Limited; Register-ScheduledTask -TaskName $task -Action $action -Principal $principal -Force | Out-Null; try { Start-ScheduledTask -TaskName $task; $deadline=[DateTime]::UtcNow.AddSeconds(${seconds + 15}); do { Start-Sleep -Seconds 1 } while(-not (Test-Path $donePath) -and [DateTime]::UtcNow -lt $deadline); $ended=[DateTime]::UtcNow; if(-not (Test-Path $donePath)){ throw 'Scheduled Task recording timed out (done marker not written)' }; if(-not(Test-Path $startedPath)){ throw 'Scheduled Task did not write recording start time' }; $recorded=(Get-Content -Raw $startedPath).Trim(); Write-Output ('recording_started_utc=' + $recorded); Write-Output ('recording_ended_utc=' + $ended.ToString('o')); Write-Output 'ffmpeg_log_begin'; if(Test-Path $log){ Get-Content -Raw $log }; Write-Output 'ffmpeg_log_end' } finally { Unregister-ScheduledTask -TaskName $task -Confirm:$false -ErrorAction SilentlyContinue }`;
